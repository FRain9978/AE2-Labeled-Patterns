...
import org.ae2PatternTagger.MSettings;
...
import org.ae2PatternTagger.network.AdvancedPatternAccessTerminalPacket;
import org.ae2PatternTagger.network.ClearAdvancedPatternAccessTerminalPacket;
            ...
            switch (moveConvenience){
                ...
                case ONCE_FOR_ALL: {
                    ...
                    if (blankPatternCount <= 0) {
                        AELog.warn("Player has no blank patterns in inventory, cannot use ONCE_FOR_ALL move convenience");
                        return;
                    }
                    int insertedCount = 0;
                    ...
                    for (ContainerTracker container : this.byId.values()) {
                        if (!container.server.isEmpty()) {
                            if (insertPatternToContainer(player, playerInventorySlot, container.server, carried.copy(), false)) {
                                ...
                                AELog.warn("Failed to insert pattern to container %s", container.container);
                            }
                        }
                    }
                    if (insertedCount > 0) {
                        ...
                        AELog.warn("Failed to insert pattern to any container");
                    }
                    return;
                }
